        ...
        <div className={`max-w-[70%] ${isFromAgent ? 'items-end' : 'items-start'} flex flex-col`}>
          ...
          >
            ...
            {showActions && (onReply || onEdit || onDelete || onInfo) && (
              <div className={`absolute top-0 ${isFromAgent ? 'left-0 -translate-x-full' : 'right-0 translate-x-full'} flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity`}>
                {onReply && (
                  <Button
                    variant="ghost"
                    size="icon"
